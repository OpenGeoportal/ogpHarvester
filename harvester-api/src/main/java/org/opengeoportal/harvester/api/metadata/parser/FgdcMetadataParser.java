...
public class FgdcMetadataParser extends BaseXmlMetadataParser {
    ...
    public static enum FgdcTag implements Tag {
        Title("title", "/metadata/idinfo/citation/citeinfo/title"),
        Abstract("abstract", "/metadata/idinfo/descript/abstract"),
        LayerName("ftname", "/metadata/idinfo/citation/citeinfo/ftname"),
        Publisher("publish", "/metadata/idinfo/citation/citeinfo/pubinfo/publish"),
        Originator("origin", "/metadata/idinfo/citation/citeinfo/origin"),
        WestBc("westbc", "/metadata/idinfo/spdom/bounding/westbc"),
        EastBc("eastbc", "/metadata/idinfo/spdom/bounding/eastbc"),
        NorthBc("northbc", "/metadata/idinfo/spdom/bounding/northbc"),
        SouthBc("southbc", "/metadata/idinfo/spdom/bounding/southbc"),
        Access("accconst", "/metadata/idinfo/accconst"),
        // Keywords
        PlaceKeywordsHeader("place", "/metadata/idinfo/keywords/place"),
        ThemeKeywordsHeader("theme", "/metadata/idinfo/keywords/theme"),
        PlaceKeywordsThesaurus("placekt", "/metadata/idinfo/keywords/place/placekt"),
        ThemeKeywordsThesaurus("themekt", "/metadata/idinfo/keywords/theme/placekt"),
        PlaceKeywords("placekey", "placekey"),  //relative xpath
        ThemeKeywords("themekey", "themekey"),  //relative xpath
        //Data type
        DataType_Srccitea("srccitea", "/metadata/spdoinfo/srccitea"), // TODO: Verify xpath
        DataType_Direct("direct", "/metadata/spdoinfo/direct"),
        DataType_Sdtstype("sdtstype", "/metadata/spdoinfo/ptvctinf/sdtsterm/sdtstype"),
        Date_Caldate("caldate", "/metadata/idinfo/timeperd/timeinfo/sngdate/caldate"),
        Date_Begdate("begdate", "/metadata/idinfo/timeperd/timeinfo/rngdates/begdate"),
        Date_DateStamp("dateStamp", "/metadata/idinfo/timeperd/timeinfo/dateStamp"); // TODO: Verify xpath;
    ...
    protected void handleOriginator() {
        ...
        try {
            this.metadataParserResponse.getMetadata().setOriginator(getDocumentValue(tag));
        } catch (Exception e) {
    ...
    protected void handlePublisher() {
        ...
        try {
            this.metadataParserResponse.getMetadata().setPublisher(getDocumentValue(tag));
        } catch (Exception e) {
        ...
        try {
            ...
            try {
                ...
                if (dateValue.length() == 4){
                    this.metadataParserResponse.getMetadata().setContentDate(processDateString(dateValue));
                }
            } catch (Exception e1) {
                logger.warn("No valid Content Date could be found in the document.");
                this.metadataParserResponse.getMetadata().setContentDate(null);
    ...
    protected void handleDataType() {
        ...
        try {
            ...
            if (srcCiteA.equalsIgnoreCase("Paper Map")) {
                this.metadataParserResponse.getMetadata().setGeometryType(GeometryType.ScannedMap);
                return;
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (direct.equalsIgnoreCase("raster") == true) {
                this.metadataParserResponse.getMetadata().setGeometryType(GeometryType.Raster);
                return;
            }
        } catch (Exception e) {
    ...
    protected void handleKeywords() {
        ...
        try {
            // Theme keywords
            NodeList themeKeywordNodes = (NodeList) xPath.evaluate(FgdcTag.ThemeKeywordsHeader.getXPathName(), document, XPathConstants.NODESET);

            for(int i = 0 ; i < themeKeywordNodes.getLength(); i++) {
                Node keyword = themeKeywordNodes.item(i);

                NodeList keywordValueNodes = (NodeList) xPath.evaluate(FgdcTag.ThemeKeywords.getXPathName(), keyword, XPathConstants.NODESET);
                if (keywordValueNodes.getLength() > 0) {
                    String keywordThesaurus = (String) xPath.evaluate(FgdcTag.ThemeKeywordsThesaurus.getXPathName(), keyword, XPathConstants.STRING);
                    ...
                    for(int j = 0 ; j < keywordValueNodes.getLength(); j++) {
                        String keywordValue = keywordValueNodes.item(j).getTextContent();
            ...
            // place keywords
            NodeList placeKeywordNodes = (NodeList) xPath.evaluate(FgdcTag.PlaceKeywordsHeader.getXPathName(), document, XPathConstants.NODESET);

            for(int i = 0 ; i < placeKeywordNodes.getLength(); i++) {
                Node keyword = placeKeywordNodes.item(i);

                NodeList keywordValueNodes = (NodeList) xPath.evaluate(FgdcTag.PlaceKeywords.getXPathName(), keyword, XPathConstants.NODESET);
                if (keywordValueNodes.getLength() > 0) {
                    String keywordThesaurus = (String) xPath.evaluate(FgdcTag.PlaceKeywordsThesaurus.getXPathName(), keyword, XPathConstants.STRING);
            ...
            this.metadataParserResponse.getMetadata().setThemeKeywords(themeKeywordList);
            this.metadataParserResponse.getMetadata().setPlaceKeywords(placeKeywordList);
        } catch (Exception e) {
